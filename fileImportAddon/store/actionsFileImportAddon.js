 ...
 * @returns {void}
 */
async function addLayerToTree (layerName, layerId, features, dispatch) {
    for (const feature of features) {
        if (!feature.get("masterportal_attributes")) {
    ...
    const newLayer = layerCollection.getLayerById(layerId);

    newLayer.getLayerSource().addFeatures(features);
    newLayer.getLayer().set("alwaysOnTop", true);
}
 ...
 * @returns {void}
 */
function addImportedFilename ({state, commit}, fileName) {
...
/**
 * Imports features from a given file source and adds them as a new layer.
 * @param {Object} context - Vuex action context.
 * @param {Object} datasrc - source object containing file data and metadata.
 * @returns {void}
 */
function importKML ({state, dispatch}, datasrc) {
    const
    ...
    addLayerToTree(layerName, layerId, features, dispatch);
}

/**
 * Sets the selected import file type.
 * @param {Object} context - Vuex action context.
 * @param {String} newFiletype - selected file type key.
 * @returns {void}
 */
function setSelectedFiletype ({commit}, newFiletype) {

...
const generateDataArray = {
     ...
     * @param {Object} dataFromEndpoint The data from the endpoint respectively from the state
    ...
    generateDataArray (dataFromEndpoint, backgroundColor, endpoint, chartType) {
        ...
        // Here we set the specific OGCAPI, groupBy property
        ...
        dataFromEndpoint?.data.forEach((element) => {
            if (endpoint === "visitors") {
                if (dataFromEndpoint.data.length > 1) {
                    ...
                    labels.push(i18next.t("additional:modules.tools.vpidashboard.tab.compareDates.dropdown.visitors"));
                }
        ...
        if (endpoint === "gender") {
            labels = sortArrays.sortGenderArray(labels);
        }
        if (endpoint === "ageGroup") {
        ...
        if (!["visitors", "daily"].includes(endpoint)) {
            labels.forEach(l => {

 ...
 */
export function calculateLineLengths (projection, lines, radius, accuracy, selectedLineStringUnit, lineStringUnits) {
    return Object.keys(lines).reduce((accumulator, lineKey) => {
        ...
        if (length.toFixed(0) === "0") {
            ...
            accumulator[lineKey] = accuracy === "decimeter" || (accuracy === "dynamic" && length < 10)
            ...
            accumulator[lineKey] = accuracy === "decimeter" || (accuracy === "dynamic" && unitLength < 10)
...
/**
 * Calculates areas of a polygon array.
 * @param {String} projection EPSG projection code
 * @param {module:ol/geom/Polygon[]} polygons polygons to calculate area of
 * @param {Number} radius earth radius to assume for calculation
 * @param {String} accuracy accuracy of the measurement result
 * @param {String} selectedPolygonUnit index of unit as string
 * @param {String[]} polygonUnits list of supported units
 * @return {MeasureCalculation[]} calculated value for display, if value is 0, return without unit
 */
export function calculatePolygonAreas (projection, polygons, radius, accuracy, selectedPolygonUnit, polygonUnits) {
    return Object.keys(polygons).reduce((accumulator, polygonKey) => {
        ...
        if (area.toFixed(0) === "0") {
            ...
            accumulator[polygonKey] = accuracy === "decimeter" || (accuracy === "dynamic" && area < 10)
            ...
            accumulator[polygonKey] = accuracy === "decimeter" || (accuracy === "dynamic" && unitArea < 10)

...
describe("addons/shared/js/mapfishUtils/collectFeatures.js", () => {
    ...
    describe("collectFeatures", () => {
        ...
        it("should call not onsuccess with oaf if no array is given", () => {
            const onsuccess = sinon.spy(),
                getOAFFeatureGetStub = sinon.stub(getFeature, "getOAFFeatureGet").resolves("foo");
        ...
        it("should call onsuccess with oaf if array is given", async () => {
            const onsuccess = sinon.stub(),
                getOAFFeatureGetStub = sinon.stub(getFeature, "getOAFFeatureGet").resolves([]),
                readAllOAFToGeoJSONStub = sinon.stub(getFeature, "readAllOAFToGeoJSON").returns(["foo"]);

            await collectFeatures({}, {}, {mapProjection: undefined, oafCRSURI: undefined}, {typ: "OAF"}, onsuccess);

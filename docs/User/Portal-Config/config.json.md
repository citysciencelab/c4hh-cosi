...
This tool allows users to compare two map layers side by side using a layer swiper. Users select layers from the active, visible layers, and the swiper divides the map to show each layer in separate sections. The tool doesn't support GROUP layers.
...
|enableUndoRedo|no|Boolean|false|Enables the measurement list with per-point undo/redo for active sketches and completed measurements. Not available in 3D mode.|false|
|icon|no|String|"bi-arrows-angle-expand"|Icon that is shown in front of the module-name in the menu. For selection see **[Bootstrap Icons](https://icons.getbootstrap.com/)**.|false|
...
|geometryName|no|String||Name of the geometry field for specialWfs geosearch.|false|
|labelProperty|no|String||(`SPECIALWFS` only) Name of the field from which the dropdown labels should be read. If not set, the first attribute from `propertyNames` is used.|false|
...
|featurePropertiesValues|no|[featurePropertiesValues](#portalconfigmenusectionsmoduleswfsttransactionconfig)[]|[]|Set default value for properties of WFS features.|false|

...
describe("SelectionManager.vue", () => {
        ...
        Portalconfig: {
            menu: {
                tools: {
                    children: {
                        SelectionManager: {
                            name: "translate#additional:modules.selectionManager.title",
                            icon: "bi-sliders2"
                        }
                    }
                }
    ...
    beforeEach(() => {
        ...
        store.commit("Modules/SelectionManager/setActive", true);
    });
    ...
    describe("Component DOM", () => {
        ...
        it("should not render if selections is not empty and dashboard is open", async () => {
            store.dispatch("Modules/SelectionManager/addNewSelection", {selection: selectionMock.selection, source: selectionMock.source, id: selectionMock.id});
        ...
        it("should set activeSelection to selection.length -1 everytime a new selection is added", async () => {
            ...
            await store.dispatch("Modules/SelectionManager/addNewSelection", {selection: selectionMock.selection, source: selectionMock.source, id: selectionMock.id});
    ...
    describe("watchers", () => {
        ...
        it("should call addSelection and highlightSelection when state.acceptSelection changes", async () => {
            ...
            await store.commit("Modules/SelectionManager/setAcceptSelection", selection);
            await wrapper.vm.$nextTick();
    ...
    describe("User Interactions", () => {
        ...
        it("should set selection.storedLayers to activeLayerList on freeze_btn click", async () => {
            ...
            expect(store.getters["Modules/SelectionManager/selections"][0].storedLayers).to.deep.equal(wrapper.vm.activeLayerList);
        });
        ...
        it("should activate an rerender the buffer on the selection on active_btn click", async () => {
            const wrapper = await mount(),
                ...
                selection = store.getters["Modules/SelectionManager/selections"][0];
        ...
        it("should remove selection from this.selections on remove_btn click", async () => {
            const wrapper = await mount(),
                ...
                selectionsArrayLength = store.getters["Modules/SelectionManager/selections"].length;

            remove_btn.trigger("click");
            await wrapper.vm.$nextTick();
            expect(spyRemoveSelection.calledOnce).to.be.true;
            expect(store.getters["Modules/SelectionManager/selections"].length).to.equal(selectionsArrayLength - 1);
        });
        ...
        it("should reset the SelectionManager on remove_all_btn click", async () => {
            ...
            remove_btn.trigger("click");
            await wrapper.vm.$nextTick();
            expect(store.getters["Modules/SelectionManager/selections"].length).to.equal(0);
            expect(wrapper.find("#sm").exists()).to.be.false;
        });
    });
});

...
export default {
    ...
    watch: {
        aSumUpSwitchA (value) {
            if (value) {
                this.selectedFieldA = [this.selectedFieldA];
            }
        },
        aSumUpSwitchB (value) {
            if (value) {
    ...
    methods: {
         ...
         */
        async checkSumUp (letter) {
            if (!this[letter + "Switch"]) {
                const mappingJson = await getMappingJson(),
                    checkSumUp = mappingJson.find(x => x.value === this["selectedField" + letter].id);
        ...
        coverageFunction (letter) {
            ...
            this.selectedFeatures.forEach(district => {
                ...
                if (this[letter + "Switch"]) {
                            ...
                            if (geometry.intersectsCoordinate(layerGeometry)) {
                                if (this["paramField" + letter].name !== "Anzahl") {
                                    if (
                                        ...
                                        const
                                            value = feature.getProperties()[this["paramField" + letter]],
                    ...
                    if (Array.isArray(this["selectedField" + letter])) {
                        this["selectedField" + letter].forEach(id => {
...
<template lang="html">
    ...
    >
        <div class="addon_wrapper">
            ...
            >
                ...
                <template v-else>
                    <v-autocomplete
                        ...
                        :items="aSumUpSwitchA ? subFeaturesList : featuresListNew"
                        dense
                        outlined
                        ...
                        @update:model-value="checkSumUp('A')"
                    />
                </template>

                <div
                    v-if="selectedFieldA.length > 0"
                    class="subsection"
                >
                    <template v-if="ASwitch">
            ...
            >
                ...
                <template v-if="BSwitch">
                    <v-select
                        ...
                        @update:model-value="getFacilityData('B')"
                    />
                </template>
                <template v-else>
                    <v-autocomplete
                        ...
                        v-model="selectedFieldB"
                        class="feature_selection selection"
                        :items="aSumUpSwitchB ? subFeaturesList : featuresListNew"
                        dense
                        outlined
                        ...
                        :placeholder="$t('additional:modules.tools.cosi.calculateRatio.placeholderB')"
                        :menu-props="{ closeOnContentClick: true }"
                        @update:model-value="checkSumUp('B')"
                    />
                </template>
                <div
                    v-if="selectedFieldB.length > 0"
                    class="subsection"
                >
                    <template v-if="BSwitch">
                        <div class="sub_wrapper">
            ...
            <div
                v-if="selectedFieldA.length > 0 && selectedFieldB.length > 0"
            ...
            >
                <div class="btn_grp finalization">
                    <FlatButton
                        ...
                        :text="$t('additional:modules.tools.cosi.calculateRatio.swapFields')"
                        @click="switchSelection"
                    />
                    <FlatButton
                        class="cancel"
                        icon="bi bi-x"
                        :text="$t('additional:modules.tools.cosi.calculateRatio.reset')"
                        @click="clearAllValues"
                    />
                    <FlatButton
                        class="confirm"
                        icon="bi bi-check2"
                        :text="$t('additional:modules.tools.cosi.calculateRatio.calculateTooltip')"

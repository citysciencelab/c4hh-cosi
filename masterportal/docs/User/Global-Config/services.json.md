...
| displayFormat | no | String | | A datetime [format-string](https://day.js.org/docs/en/display/format) to format the timestamp label in the Timeslider. If not specified ISO 8601 format is used. | "DD.MM.YYYY HH:mm" |
| displayTimezone | no | String | | A timezone to transform the timestamp label in the Timeslider. The configured timezone is only used if a format is configured. | "Europe/Berlin" |
| dualRangeSlider | no | Boolean | `false` | This attribute allows the Timeslider to be configured as a dual range slider. | `true` |
...
| name | yes | String |  | Arbitrary display name used in the layer tree. | `"Traffic situation on freeways"` |
| params | no | String{} |  | Key value pairs to add to the request. |  |

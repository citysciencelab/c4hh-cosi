...
/**
 * Adds an imported vector layer to the map and layer collection.
 * Keeps the feature style from the imported file if present.
 *
 * @param {{name: string, id: string, filename?: string, features: ol.Feature[]}} newLayer - imported layer
 * @returns {Object} model-like representation of the added layer
 */
function addLayerToTree (newLayer) {
    const {name, id, filename, features} = newLayer,
        hasFeatureStyles = features.some(f => typeof f.getStyle === "function" && f.getStyle()),
        olLayer = new VectorLayer({
            source: new VectorSource({features}),
            visible: true,
            style: hasFeatureStyles ? null : undefined
        }),
        mpLayer = {
            get: key => ({typ: "WFS", id, name, filename}[key]),
            getLayer: () => olLayer,
            attributes: {id, name, typ: "WFS", filename},
            layer: olLayer
        },
        map = mapCollection?.getMap?.("2D"),
        extent = olLayer.getSource()?.getExtent();

    olLayer.setZIndex(10000);
    olLayer.set("filename", filename);

    if (!layerCollection.getLayerById(id)) {
        layerCollection.addLayer(mpLayer);
    }
    else {
        // @TODO show in GUI
        console.warn(`Layer with ID "${id}" already exists in layerCollection.`);
    }

    if (map && !map.getLayers().getArray().includes(olLayer)) {
        map.addLayer(olLayer);
    }

    if (extent && extent[0] !== Infinity) {
        map?.getView()?.fit(extent, {duration: 800, maxZoom: 16});
    }
...
export default {
    passLayer ({commit, dispatch, rootGetters}, newLayer) {
        ...
        const
            ...
            filters = rootGetters["Modules/Filter/layers"];

        commit("Modules/Filter/setLayers", [...filters, filter], {root: true});
        commit("setUpdateLayerStyles", true);
        return model;
    },
    /**
     * Removes an imported layer from the map and internal layer collection.
     * Ensures both OpenLayers and Masterportal references are properly cleaned up.
     * @param {Object} context - Vuex context object.
     * @param {Object} context.state - Current Vuex module state.
     * @param {Function} context.commit - Vuex commit function.
     * @param {String} filename - Name of the imported file whose layer should be removed.
     * @returns {void}
     */
        ...
        try {
            const map = mapCollection?.getMap?.("2D"),
                ...
                targetLayer = layers.find(l => {
                    const layerFilename = l.get("filename") || l.get("name") || "";

                    return layerFilename === filename;
                }),
                mpLayer = typeof layerCollection?.getLayerWhere === "function"
                    ? layerCollection.getLayerWhere({filename}) || layerCollection.getLayerWhere({name: filename})
                    : null,
                updatedFiles = (state.importedFileNames || []).filter(f => f !== filename);
            ...
            if (mpLayer && typeof layerCollection?.removeLayer === "function") {
                layerCollection.removeLayer(mpLayer);
            }

            commit("setImportedFileNames", updatedFiles);
        }
        catch (err) {
            console.error("Error while removing layer:", err);
        }
    },
        ...
        features.forEach(feature => {
            ...
            if (feature.getGeometry() === null) {
                ...
                geometries.forEach(geometry => {
                    const coords = geometry.getFirstCoordinate?.() || geometry.getCoordinates?.(),
                        dest = currentCrs;

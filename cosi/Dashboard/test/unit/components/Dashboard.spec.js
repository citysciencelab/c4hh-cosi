...
describe.skip("addons/cosi/Dashboard/components/Dashboard.vue", () => {
    ...
    beforeEach(async () => {
        ...
        store.commit("Tools/Dashboard/setItems", []);
    ...
    describe("Calculation functions", () => {
        ...
        it("create new table item by divisionPercent", async () => {
            const spyCalculateAll = sinon.spy(Dashboard.methods, "calculateAll"),
                spyUpdateDistricts = sinon.spy(Dashboard.methods, "updateDistricts");

            await factory.initialize();
            store.commit("Tools/Dashboard/setCalculations", []);
            await wrapper.vm.$nextTick();
            // act
            await wrapper.findAll(".open-burger-menu").at(0).trigger("click");
            await wrapper.findAll(".activator").at(0).trigger("click");
            ...
            await wrapper.findAll(".activator").at(1).trigger("click");
            await wrapper.find("#dividePercent").trigger("click");
            await wrapper.find("#confirm-calc").trigger("click");

            await wrapper.vm.$nextTick();
            expect(wrapper.vm.calculations).to.have.lengthOf(1);
            expect(spyCalculateAll.callCount).to.equal(2);
            expect(spyUpdateDistricts.callCount).to.equal(1);
        });
    });
    ...
    describe("Methods", () => {
        ...
        describe("getFixedHeader", async () => {
            it("should return the default data", async () => {
                factory.initialize();

                const def = ["Kategorie", "Jahr", "Gesamt", "Durchschnitt"];

                expect(await wrapper.vm.getFixedHeader("")).to.deep.equal(def);
                expect(await wrapper.vm.getFixedHeader(true)).to.deep.equal(def);
                expect(await wrapper.vm.getFixedHeader([])).to.deep.equal(def);
                expect(await wrapper.vm.getFixedHeader({})).to.deep.equal(def);
                expect(await wrapper.vm.getFixedHeader(0)).to.deep.equal(def);
                expect(await wrapper.vm.getFixedHeader({}, {})).to.deep.equal(def);
            });

            it("should return the correct data for header", async () => {
                factory.initialize();

                const selectedDistrictNames = ["21002", "21003"],
                    selectedColumns = [
                        {
                            "text": "21002",
                            "value": "21002",
                            "align": "end",
                            "district": {},
                            "districtLevel": "Statistische Gebiete",
                            "sortable": false,
                            "groupable": false,
                            "selected": false,
                            "minimized": false
                        ...
                        {
                            "text": "Altona-Altstadt",
                            "value": "Altona-Altstadt",
                            "align": "end",
                            "district": {},
                            "districtLevel": "Stadtteile",
                            "sortable": false,
                            "groupable": false,
                            "selected": false,
                            "minimized": false,
                    ...
                    header = await wrapper.vm.getFixedHeader(selectedDistrictNames, selectedColumns),
                    expected = [
                        "Kategorie",
                        "Jahr",
                        "21002",
        ...
        describe("getModifiedData", () => {
            it("should return the default data", () => {
                ...
                expect(wrapper.vm.getModifiedData("")).to.be.equal("");
                expect(wrapper.vm.getModifiedData(true)).to.be.true;
                expect(wrapper.vm.getModifiedData([])).to.be.deep.equal([]);
                expect(wrapper.vm.getModifiedData({})).to.be.deep.equal({});
                expect(wrapper.vm.getModifiedData(0)).to.be.equal(0);
                expect(wrapper.vm.getModifiedData([{"oldKey": true}], 0)).to.be.deep.equal([{"oldKey": true}]);
                expect(wrapper.vm.getModifiedData([{"oldKey": true}], true)).to.be.deep.equal([{"oldKey": true}]);
                expect(wrapper.vm.getModifiedData([{"oldKey": true}], [])).to.be.deep.equal([{"oldKey": true}]);
                expect(wrapper.vm.getModifiedData([{"oldKey": true}], {})).to.be.deep.equal([{"oldKey": true}]);
                expect(wrapper.vm.getModifiedData([{"oldKey": true}], 0)).to.be.deep.equal([{"oldKey": true}]);
            });

            it("should return the data with modified key", () => {
                factory.initialize();

                const data = [
                        {"key1": 0, "key2": 1},
                        {"key1": 0, "key2": 1, "oldKey": 2}
                    ],
                    modifiedKey = [{"oldKey": "oldKey", "newKey": "newKey"}],
                    newData = [
                        {"key1": 0, "key2": 1},
                        {"key1": 0, "key2": 1, "newKey": 2}
                    ];

                expect(wrapper.vm.getModifiedData(data, modifiedKey)).to.be.deep.equal(newData);
            });
        });
    });
    */

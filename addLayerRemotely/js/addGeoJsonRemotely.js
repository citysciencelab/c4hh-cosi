...
function getFeatureIds (layerId) {
    ...
    if (typeof layer === "undefined") {
        console.warn(i18next.t("additional:modules.addGeoJsonRemotely.layerNotFound", {layerId: layerId}));
        return featureArray;
    }
...
/**
 * Transform a features' reference system
 * @param {ol.Feature} feature a feature
 * @param {String} projectionCode the EPSG code, the feature shall be transformed to
 * @return {ol.Feature} The updated feature
 */
function transformFeature (feature, projectionCode) {
    const
        geometry = feature.getGeometry();

    let
        referenceSystem = feature.get("referenceSystem");

    referenceSystem = referenceSystem === undefined ? "4326" : referenceSystem;

    if (geometry) {
        geometry.transform("EPSG:" + referenceSystem, projectionCode);
    }
    return feature;
}

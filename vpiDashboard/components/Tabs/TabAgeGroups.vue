        ...
        return {
            ...
            pieChartOptions: {
                ...
                plugins: {
                    tooltip: {
                        callbacks: {
                            // Creates a AgeGroups Tooltip like "30-39: 30.9%"
                            label: (tooltipItem) => {
    ...
    methods: {
        ...
        async dayDatepickerValueChanged (values) {
            if (values[0] && values[1]) {
                this.setLoader(true);
                const responses = await apiEndpointService.receiveAgeGroups(
        ...
        createChartData (responseData, chartType) {
                ...
                this.ageGroupChartLabels.forEach((label, index) => {
                    if (!tempData[index]) {
                        tempData[index] = {
                            ...
                            hoverOffset: 4,
                            fill: chartType === "bar",
                            backgroundColor: this.ageGroupsColors[label],
                            label,
                            borderColor: this.ageGroupsColors[label]
                        };
                    }
                    if (element.properties.altersklasse === `[${label}]`) {
            ...
            chartType === "bar" ? this.setAllAgeGroupsDailyData(tempData) : this.setAllAgeGroupsDailyDataLine(tempData);
            this.setAgeGroupDailyXLabels([...xLabels]);
        },
        /**
        ...
        async getCurrentChartsData () {
            ...
            this.chartdata.pie.labels = this.ageGroupChartLabels;
            ...
            switch (this.selectedChartData) {
                ...
                case "monthlyoverview":
                    this.updateChartValues(
                        this.allAgeGroupsMonthlyData[this.currentlySelectedYear],
                        this.allAgeGroupsMonthlyDataLine[this.currentlySelectedYear],
                        this.ageGroupMonthlyXLabels
                    );
                    break;
                default:
                    this.updateChartValues();
        ...
        setChartData (features) {
            ...
            this.updateChartValues(this.allAgeGroupsDailyData, this.allAgeGroupsDailyDataLine, this.ageGroupDailyXLabels);
        },
        /**

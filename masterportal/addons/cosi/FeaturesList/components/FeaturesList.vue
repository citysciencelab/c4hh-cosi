...
export default {
    ...
    data () {
        return {
            ...
            featureColumns: [
                ...
                {
                    title: this.$t("additional:modules.tools.cosi.featuresList.colLayerName"),
                    value: "layerName",
                    filter: value => {
                ...
                {
                    title: this.$t("additional:modules.tools.cosi.featuresList.colType"),
                    value: "type",
                    divider: true
                },
                {
                    title: this.$t("additional:modules.tools.cosi.featuresList.colGroup"),
                    value: "group",
                    divider: true
                }
            ],
    ...
    computed: {
        ...
        ...mapGetters("Modules/FeaturesList", Object.keys(getters)),
        // ...mapGetters("Modules/ScenarioBuilder", ["activeSimulatedFeatures", "scenarioUpdated", "geomAttributes"]),
    ...
    methods: {
        ...mapMutations("Modules/FeaturesList", Object.keys(mutations)),
        ...mapActions("Modules/FeaturesList", Object.keys(actions)),
        ...
        getNumericalColumns () {
            const numCols = this.getColumns(this.getActiveLayers(), "numericalValues");

            if (this.items.length && typeof this.items[0]?.score?.value !== "undefined") {
                numCols.push({
                    title: this.$t("additional:modules.tools.cosi.featuresList.distanceScore"),
                    value: "distanceScore",
                    divider: true,
                    hasAction: true,
                    invertColor: true
        ...
        /**
         * Gets column header objects by layers config.
         * Different types of column headers can be configured on the layer in the config.json
         * @param {Object[]} mappedLayersConfig - A mapped layer config list.
         * @param {String} type - The type of the header.
         * @returns {Object[]} An array of objects that each describe a header column.
         */
        getColumns (mappedLayersConfig, type) {
            if (mappedLayersConfig.length === 0) {
                return [];
            }

            const additionalColumns = mappedLayersConfig.reduce((cols, layerConfig) => {
                const _numCols = layerConfig[type].map(value => ({
                    title: value.name,
                    value: value.id
                }));
            ...
            if (additionalColumns.length > 0) {
                additionalColumns[additionalColumns.length - 1].divider = true;
            }
        ...
        searchAllAttributes (value, search, item) {
            const allProps = {...item.raw.feature.getProperties(), ...item.raw};
                        ...
                        >
                            ...
                            >
                                ...
                                <template v-if="!isNaN(parseFloat(item[col.value]))">
                                    ...
                                    >
                                        <div>
                                            {{ parseFloat(item[col.value]).toLocaleString("de-DE") }}
                                            {{ item[col] }}
                                        </div>
                                        <div>
                                            <v-chip

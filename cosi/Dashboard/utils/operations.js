...
import {getDistrictByName, getStatisticByCategory} from "../../DistrictSelector/utils/districts.js";
 ...
 * @param {"add" | "subtract" | "multiply" | "divide"  | "dividePercent" | "sumUpSelected"} operation - the mathematical operation to execute
...
export function addCalculation (operation, options = {}, id = undefined) {
    ...
    this.addSingleAlert({
        content: this.$t("additional:modules.tools.cosi.dashboard.calculationHint"),
        class: "Info",
        displayClass: "info"
    });
}
 ...
 * @param {Object} item - the data for that category
 * @param {String[]} districts - the districts to generate the data for (must be the LABELS of the districts as they are used as keys in param 'item' NOT the names)
 * @param {Number} timestamp - the current timestamp
 * @param {String} timestampPrefix - the prefix for timestamp attributes

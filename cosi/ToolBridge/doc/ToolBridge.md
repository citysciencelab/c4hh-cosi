...
this.$store.dispatch("Modules/ToolBridge/runTool", {
                toolName: "AccessibilityAnalysis",
                settings: settings,
                outputCallback: (output) => { this.$store.commit("Modules/SomeToolName/someVariable", output) }
            });
...
2. You should also add the name of your tool as a string to to `supportedTools` array in the toolBridge store (`Modules/ToolBridge/supportedTools`)
...
watch: {
    ...
    this.$store.commit("Modules/ToolBridge/setReceivedResults", // this is where toolBridge expects requested results to arrive
                    {
    ...
    toolBridgeIn (newRequest) {
        ...
        */
        const updateInterface = (request) => {
            ...
            */
            returnResults = () => {
                return this.$store.commit("Modules/ToolBridge/setReceivedResults", // this is where toolBridge expects requested results to arrive
                    {

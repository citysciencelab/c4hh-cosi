...
export default {
    ...
    */
    alertWrongInputParameters ({dispatch}) {
        dispatch("Alerting/addSingleAlert",
            "<strong>" + i18next.t("additional:modules.oktagon.wrongAddressParameter") + "</strong>"
            + "<br>"
            + "<small>" + i18next.t("additional:modules.oktagon.wrongAddressParameterMessage") + "</small>",
            {root: true}
        );
    },
    /**
    ...
    */
    initURLParameter ({dispatch, commit, getters}) {
        ...
        if (result?.BEZIRK) {
            const districtFromUrl = result.BEZIRK;
            let districtNameToZoom = "";

            districtNameToZoom = getters.hasDistrict(districtFromUrl);
            if (districtNameToZoom === "") {
                dispatch("Alerting/addSingleAlert",
                    "<strong>" + i18next.t("additional:modules.oktagon.wrongDistrictName") + "</strong>"
                    + "<br>"
                    + "<small>" + i18next.t("additional:modules.oktagon.wrongDistrictNameMessage") + districtFromUrl + ".</small>",
                    {root: true}
                );
            }
            else {
                dispatch("Maps/zoomToFeatures", {ZOOMTOGEOMETRY: districtNameToZoom}, {root: true});
            }
        }
    ...
    */
    parseXML ({getters, commit}, xml) {
        ...
        xmlElements.forEach(xmlElement => {
            ...
            if (xmlElement.getAttribute("bezeichnung")) {
                submitObject.Gemarkungsname = xmlElement.getAttribute("bezeichnung");
            }
            if (xmlElement.getAttribute("gemarkunggemarkungsnummer")) {
                submitObject.Gemarkungsnummer = xmlElement.getAttribute("gemarkunggemarkungsnummer");
            }
            if (xmlElement.getAttribute("flurstuecksnummerzaehler")) {
                submitObject.Flurstuecksnummer = xmlElement.getAttribute("flurstuecksnummerzaehler");
            }
        });
    ...
    */
    requestALKISWMS ({dispatch}, url) {
        axios({
            ...
            dispatch("Alerting/addSingleAlert",
                "<strong>" + i18next.t("additional:modules.oktagon.fetchFailed") + "</strong> <br>"
                + "<small>" + i18next.t("additional:modules.oktagon.fetchFailedMessage") + "</small>"
            );
        });
    },
    /**
    ...
    */
    zoomToAddress ({dispatch}, coordinates) {
        ...
        if (coordinates && coordinates !== null && Array.isArray(coordinates)) {
            ...
            dispatch("Maps/setZoom", 9, {root: true});
            dispatch("Maps/placingPointMarker", coordinatesArray, {root: true});
        }
        else {

...
import getters from "../../../store/gettersOktagon.js";
...
describe("addons/oktagon/store/gettersOktagon", () => {
    ...
    it("hasDistrict handels the Bezirk URL parameter", () => {
        const districtFromUrl = "HARBURG",
            rootGetters = {
                zoomTo: [
                    {
                        allowedValues: ["HARBURG", "EIMSBÜTTEL", "ALTONA"]
                    }
                ]
            };

        expect(
            hasDistrict(null, null, null, rootGetters)(districtFromUrl)
        ).to.equals("HARBURG");
    });
});
